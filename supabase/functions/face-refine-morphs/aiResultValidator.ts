...
export async function validateAndClampAIResults(aiResult, mappingData, k5_envelope, resolvedGender, vision_classification, traceId) {
  ...
  if (!aiResult.final_shape_params || typeof aiResult.final_shape_params !== 'object') {
    throw new Error('AI result missing or invalid final_shape_params');
  }
 ...
 */ function applyEnhancedSemanticCoherence(shapeParams, limbMasses, resolvedGender, k5_envelope, vision_classification1, traceId) {
  const corrections = [];
  ...
  const isObese = vision_classification1.obesity === 'Obèse' || vision_classification1.obesity === 'Obésité morbide';
  const isOverweight = vision_classification1.obesity === 'Surpoids';
