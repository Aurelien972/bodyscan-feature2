...
import CapturedPhotoDisplay from './components/CapturedPhotoDisplay';
import CameraInterface from '../components/CameraInterface/CameraInterface';
...
/**
 * Body Scan Photo Capture Step - VisionOS 26 Optimized
 * Modular, fluid, and performance-optimized photo capture experience
 */
const BodyScanPhotoCaptureStep: React.FC<BodyScanPhotoCaptureStepProps> = ({
  ...
  isFaceScan = false, // NOUVEAU: Valeur par défaut
  ...
  const processPhotoCapture = useCallback(async (file: File) => {
    ...
    try {
      // Step 1: Enhanced validation and processing
      ...
      if (!formatValidation.isValid) {
        ...
        if (criticalIssues.length > 0) {
          ...
          // Non-critical issues (like non-JPEG format) - show warning but continue
          showToast({
            type: 'warning',
      ...
      // Step 2: Enhanced photo processing with compression
      ...
      // Show compression feedback if applied
      ...
      // Step 3: Validate photo content (simplified for development)
      ...
      // Step 7: Separate capture success from validation status
      const isCaptured = true; // Photo is always captured if we reach this point
      const isValidated = validationResult.isValid;
      
      // Mise à jour immédiate de la progression lors de la capture
      ...
      setTimeout(async () => {
        ...
        // Auto-scroll to next photo section after front photo capture
        if (photoType === 'front') {
          setTimeout(() => {
        ...
        // Auto-scroll to launch button after profile photo capture (if both photos are ready)
      ...
      // Feedback based on validation result
      if (isCaptured) {
        showToast({
          type: captureType,
  ...
  const handleFileSelect = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Check for very large files that might cause timeout issues
    ...
    // Legacy size check (now handled by enhanced validation)
  ...
  return (
    <div className="space-y-8 body-scan-page">
      {/* Enhanced Photos Display with Glass Aesthetics */}
          ...
          <GlassCard className="glass-card--capture glass-card--capture-front p-6 relative overflow-visible">
            ...
            {frontPhoto ? (
              ...
              <div className="space-y-6">
                <PhotoGuideOverlay type="front" isFaceScan={isFaceScan} /> {/* NOUVEAU: Passer isFaceScan */}
                ...
                {step === 'front-photo' && (
                  <PhotoCaptureControls
                    ...
                    isProgressInitialized={isProgressInitialized} // Pass the prop
                  />
                )}
              </div>
            )}
        ...
        >
          <GlassCard className="glass-card--capture glass-card--capture-profile p-6 relative overflow-visible">
            ...
            {profilePhoto ? (
              ...
              <div className="space-y-6">
                <PhotoGuideOverlay type="profile" isFaceScan={isFaceScan} /> {/* NOUVEAU: Passer isFaceScan */}
